...
def entry_list(tree, ns):
    ...
    if event_el is not None:
        for group_el in event_el.findall('iof:Class', ns):
    ...
    if root.find('iof:PersonEntry', ns):
        for person_entry_el in root.findall('iof:PersonEntry', ns):
            ...
            if org_el:
                ...
                if role_el:
                    role_person = role_el.find('iof:Person', ns)
                    ...
                    for contact_el in role_person.findall('iof:Contact', ns):
                        organization['role_person'] += ' ' + contact_el.text
            ...
            service_requests_el = person_entry_el.find('iof:ServiceRequest', ns)
            comment = ''
            if service_requests_el is not None:
                comment_el = service_requests_el.find('iof:Comment', ns)
...
def event(tree, ns):
    ...
    name_el = event_el.find('iof:Name', ns)
    if name_el is not None:
        event_obj['name'] = name_el.text

    start_time_el = event_el.find('iof:StartTime', ns)
    if start_time_el is not None:
        if start_time_el.find('iof:Date', ns) is not None:
            event_obj['start_date'] = start_time_el.find('iof:Date', ns).text

    end_time_el = event_el.find('iof:EndTime', ns)
    if end_time_el is not None:
        if end_time_el.find('iof:Date', ns) is not None:
            event_obj['end_date'] = end_time_el.find('iof:Date', ns).text

    url_el = event_el.find('iof:URL', ns)
    if url_el is not None:
        event_obj['url'] = url_el.text

    form_el = event_el.find('iof:Form', ns)
    if form_el is not None:
        event_obj['is_team_race'] = form_el.text == 'Team'

    for race_el in event_el.findall('iof:Race', ns):
        race_name_el = race_el.find('iof:Name', ns)
        race_number_el = race_el.find('iof:RaceNumber', ns)
        race_obj = {
            'name': race_name_el.text if race_name_el is not None else '',
            'number': int(race_number_el.text) if race_number_el is not None else 0,
        }
        start_time_el = race_el.find('iof:StartTime', ns)
        if start_time_el:
            if start_time_el.find('iof:Date', ns) is not None:
                race_obj['start_date'] = start_time_el.find('iof:Date', ns).text
            if start_time_el.find('iof:Time', ns) is not None:
                race_obj['start_time'] = start_time_el.find('iof:Time', ns).text
