...
def test_free_course_order_unique_controls():
    ...
    assert ok(course=['*(31-33)'], splits=[32])
...
def test_course_free_order():
    ...
    c = ['*(31-33,41)[2]', '*(71-74)[2]']
    assert ok(c, [31, 41, 71, 74])
    assert ok(c, [31, 31, 41, 71, 74])
    assert ok(c, [31, 47, 41, 71, 74])
    assert ok(c, [31, 32, 41, 71, 74])
    assert ok(c, [31, 32, 71, 72, 74])

    assert dsq(c, [31, 71, 41, 74])
    assert dsq(c, [31, 41, 74])
    assert dsq(c, [31, 44, 71, 74])
    assert dsq(c, [31, 41, 41, 74])
...
# fmt: off
@pytest.mark.parametrize(
    'score, max_time, finish, expected',
    [
        (5, (0, 1, 0, 0), (0, 0, 59, 59), 0),
        (5, (0, 1, 0, 0), (0, 1,  0,  0), 0),
        (5, (0, 1, 0, 0), (0, 1,  0,  1), 1),
        (5, (0, 1, 0, 0), (0, 1,  0, 30), 1),
        (5, (0, 1, 0, 0), (0, 1,  1,  0), 1),
        (5, (0, 1, 0, 0), (0, 1,  1,  1), 2),
        (5, (0, 1, 0, 0), (0, 1,  6,  1), 5), # penalty does not exceed total score
        (5, (0, 1, 0, 0), (0, 1, 15,  1), 5), # penalty does not exceed total score
    ],
)
# fmt: on
def test_calculate_rogaine_penalty(score, max_time, finish, expected):
    create_race()
