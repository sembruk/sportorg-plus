...
def create_multiple_teams(prepare_race):
    ...
    team.get_leg(1).person.team = get_team_by_name('Team3')
...
def create_person(name: str, team_name: str, bib: int = 0) -> Person:
    person = race().add_new_person(append_to_race=True)
    person.name = name
    person.group = get_group()
    person.bib = bib
    person.team = get_team_by_name(team_name)
...
def get_team_by_name(team_name: str) -> Team:
    org = race().find_team(team_name)
    if not org:
        org = race().add_new_team(append_to_race=True)
        org.name = team_name
    return org


def get_team_by_place(place: int) -> RelayTeam:
